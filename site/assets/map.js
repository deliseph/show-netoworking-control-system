// The module map.
//
// Navigation first: every figure across the sixteen classes, in the order they
...
if (root) {
  const seen = (() => {
    try { return JSON.parse(localStorage.getItem('snc-review') || '{}'); } catch { return {}; }
  })();

  fetch('/assets/data.json').then((r) => r.json()).then((d) => {
    const done = touched();
    // The class list comes from the build, so sixteen titles are never a
    // second copy here waiting to go stale.
    const CLASSES = d.classes.map((c) => [c.n, c.title]);
    ...
    root.append(...CLASSES.map(([n, title]) => {
      ...
      return h(`<section class="mp-class">
        <h2 class="mp-h"><a href="/session/${n}">Session ${n} · ${title}</a></h2>
